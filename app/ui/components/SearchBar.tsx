...
export default function SearchBar({ onSearch } : SearchBarProps) {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      console.log(err)
      throw new Error('Error with searching Location')
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className="w-full max-w-xlg min-w-[200px]">
      ...
      <div className="relative" role="search">
        ...
        <div id='search-description' className="sr-only">
          Type your search terms and press the search button to find the weather for your Location
        </div>
        <button
          ...
          type="button"
          aria-label={isLoading ? 'Searching Location' : 'Search Location'}>
          {isLoading ? (
